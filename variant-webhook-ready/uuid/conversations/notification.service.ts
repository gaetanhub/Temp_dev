...
export async function generateTelegramLink(memberId: string): Promise<string> {
  const botUsername = process.env.TELEGRAM_BOT_USERNAME;
  if (!botUsername) throw new Error('TELEGRAM_BOT_USERNAME manquant');

  const token = randomUUID();
  const tokenExpiresAt = new Date(Date.now() + 15 * 60 * 1000);
  await notificationDatabase.upsertTokenForMember(memberId, token, tokenExpiresAt);
